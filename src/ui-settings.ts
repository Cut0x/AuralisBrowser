/**
 * ui-settings.ts — Pages de paramètres Auralis : routeur + Apparence + Moteur + Démarrage.
 * Exporte renderAuralisContent() utilisé par ui-nav.ts via import dynamique.
 */
...
import { renderTabStrip, updateApNavItems,
         isAuralisPageVisible }                          from './ui-nav.js';
...
/** Routeur : affiche la page de paramètres correspondant au chemin. */
export function renderAuralisContent(path: string): void {
  ...
  switch (path) {
    ...
    case 'securite':   import('./ui-settings-data.js').then(({ renderPageSecurite })   => renderPageSecurite(content)); break;
...
function renderPageApparence(el: HTMLElement): void {
  ...
  langEl.addEventListener('change', () => {
    ...
    renderFavBar(); renderNewtabFavs(); renderPageApparence(el);
  });
  favbarEl.addEventListener('change', () => {
    updateSettings({ ...settings, showFavoritesBar: favbarEl.checked });
