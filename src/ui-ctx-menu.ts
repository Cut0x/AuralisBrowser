...
import { removeBookmark, renameItem, findBookmarkById }   from './bookmarks-store.js';
import { moveBookmark, getFlatFolders, extractItem }      from './bookmarks-move.js';
...
function addBtn(menu: HTMLElement, action: string, label: string, danger = false): void {
  ...
  b.className = `bm-ctx-item${danger ? ' bm-ctx-danger' : ''}`; b.textContent = label;
...
function closeCtxMenu(): void { const m = document.getElementById('bm-ctx-menu'); if (m) m.style.display = 'none'; }
...
function handleAction(action: string): void {
  ...
  if (!item) return;
  switch (action) {
    case 'open':   if (item.type === 'link') { hideAuralisPage(); navigate(item.url); } break;
    case 'rename': {
      const name = prompt('Nouveau nom :', item.type === 'link' ? item.title : item.name);
      if (!name?.trim()) return;
      updateSettings(renameItem(settings, id, name.trim()));
      saveSettings(settings); renderFavBar(); renderNewtabFavs(); refreshFavorisPage(); break;
    }
    case 'move':   showMovePicker(id); break;
    case 'delete': updateSettings(removeBookmark(settings, id)); saveSettings(settings); renderFavBar(); renderNewtabFavs(); refreshFavorisPage(); break;
  }
}
...
function showMovePicker(itemId: string): void {
  ...
  const addFolderBtn = (id: string | null, label: string) => {
    ...
    b.addEventListener('click', () => {
      if (id) { updateSettings(moveBookmark(settings, itemId, id, 'inside')); }
      else { const [ex, rest] = extractItem(settings.bookmarks, itemId); if (ex) updateSettings({ ...settings, bookmarks: [ex, ...rest] }); }
      saveSettings(settings); renderFavBar(); renderNewtabFavs(); refreshFavorisPage(); cleanup();
    });
    modal.appendChild(b);
  };
  ...
  cancel.addEventListener('click', cleanup); modal.appendChild(cancel);
